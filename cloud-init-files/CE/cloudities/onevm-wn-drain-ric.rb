...
def main
  ...
  OptionParser.new do |op|
    ...
    op.on('-p', '--hyp HYPERVISOR', "Hypervisor name") do |hyp|
      puts "Cleaning Hypervisor = #{hyp}"
      nodi = %x(onevm-addr.rb | grep #{hyp} | cut -d"," -f2 | cut -d"|" -f1)
      nodi.each do |nome|
        if nome.match("t2-vwn-")
          nodes << {
            :name => nome.strip,
            :errcount => 0,
          }
